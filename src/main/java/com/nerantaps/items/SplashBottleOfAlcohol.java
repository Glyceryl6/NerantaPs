...
public class SplashBottleOfAlcohol extends Item {

    public SplashBottleOfAlcohol() {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if (!level.isClientSide) {
            ThrownSplashBottleOfAlcohol alcohol = new ThrownSplashBottleOfAlcohol(player, level);
            alcohol.setItem(itemInHand);
            alcohol.shootFromRotation(player, player.getXRot(), player.getYRot(), 0.0F, 1.5F, 1.0F);
            level.addFreshEntity(alcohol);
        }
